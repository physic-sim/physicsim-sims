...
export default class CircularMotionSimulation extends Simulation {
  ...
  constructor(container, inputs, graphs, controls, attributes) {
	...
	let onClickV = () => {this.toggleInputPref('tangential')};
	...
	let onClickA = () => {this.toggleInputPref('angular')};
	...
	this.vInput = new ValueInput(this.wrapperV, 10, 'Tangential Velocity', 'ms<sup>-1</sup>');
	this.wrapperA = document.createElement('div');
	this.omegaInput = new ValueInput(this.wrapperA, 0, 'Angular Velocity', 'rads<sup>-1</sup>');
	...
	this.sChart = this.initChart(this.sGraph, 'Displacement-Time (x-direction)', 'sₓ (m)');
    this.vChart = this.initChart(this.vGraph, 'Velocity-Time (x-direction)', 'vₓ (m/s)');
    this.aChart = this.initChart(this.aGraph, 'Acceleration-Time (x-direction)', 'aₓ (m/s²)');
  }
  ...
  init(p) {
	// add custom btns
	this.toggleFrameRefBtn = new Button(this.controlWrapper, (() => this.toggleFrameRef(p)).bind(this), 'Frame Ref.')
  ...
  frame(p) {
	...
	// draw arrows
	this.drawArrow(p, this.pos, p5.Vector.mult(av, this.mass / p.getTargetFrameRate()), 3, 2, [0, 255, 0])
	this.drawArrow(p, this.pos, p5.Vector.mult(vpv, 1 / p.getTargetFrameRate()), 3, 10, [255, 0, 0])
	this.drawArrow(p, this.c, p5.Vector.add(this.c, p.createVector(this.pos.x, 0, 0)), 0, 1, [108, 70, 204]);

	if (this.rotationalRef) {
	  	this.drawArrow(p, this.pos, p5.Vector.mult(av, -1 * this.mass / p.getTargetFrameRate()), 3, 2, [0, 255, 0])
	}
