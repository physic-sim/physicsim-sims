...
export default class ProjectileSimulation extends Simulation {
	...
	constructor(container, inputs, graphs, controls, attributes) {
		...
		this.sizeInput = new SliderInput(this.inputWrapper, this.size, 10, 500, 1, 'Simulation Size', 'm')
		...
		this.sChart = this.initChart(this.sGraph, 'Displacement-Time (z-direction)', 's (m)');
    	this.vChart = this.initChart(this.vGraph, 'Velocity-Time (z-direction)', 'v (m/s)');
    	this.aChart = this.initChart(this.aGraph, 'Acceleration-Time (z-direction)', 'a (m/s²)');
	}
	...
	init(p) {
		...
		this.particle = new Particle(this.mass.get(), this.s.get(), this.u.get(), p.createVector(0, this.G, 0))
	}
	...
	frame(p) {
		...
		if (this.particle.pos.x < -this.size || this.particle.pos.x > this.size) {
			this.paused = true;
		}
		
		if (this.particle.pos.z < -this.size || this.particle.pos.z > this.size) {
			this.paused = true;
		}
...
class Particle {
	...
	update(p, e) {
		...
		let aTSquared = p5.Vector.mult(this.a, 0.5 * Math.pow(1/p.getTargetFrameRate(), 2))
