...
export default class CollisionsSimulation extends Simulation {
	...
	constructor(container, inputs, graphs, controls, attributes) {
		...
		this.velA = new VectorInput(this.inputWrapper, 2.5, 0, 0, 'v<sub>A</sub>', 'm/s');
		this.velB = new VectorInput(this.inputWrapper, -2.5, 0, 0, 'v<sub>B</sub>', 'm/s');
	
		this.posA = new VectorInput(this.inputWrapper, -20, 0, 0, 's<sub>A</sub>', 'm');
		this.posB = new VectorInput(this.inputWrapper, 20, 0, 0, 's<sub>B</sub>', 'm');
	
		this.sizeInput = new SliderInput(this.inputWrapper, this.size, 5, 100, 1, 'Simulation Size', 'm');
		this.eInput = new SliderInput(this.inputWrapper, 1, 0, 1, 0.01, 'Coeffecient of Resitution', '');
		...
		this.xChart = this.initChart(this.xGraph, 'Momentum (x) in System', 'kg/ms⁻¹')
		this.yChart = this.initChart(this.yGraph, 'Momentum (y) in System', 'kg/ms⁻¹')
		this.zChart = this.initChart(this.zGraph, 'Momentum (z) in System', 'kg/ms⁻¹')
	}
	...
	init(p) {
		// add custom btns
		this.downloadBtn = new Button(this.controlWrapper, this.download.bind(this), 'Data');
		...
		// init particles
		this.particleA = new Particle(this.massA.get(), this.posA.get(), this.velA.get(), 166, 189, 111);
		this.particleB = new Particle(this.massB.get(), this.posB.get(), this.velB.get(), 156, 98, 112);
	}
	...
	frame(p) {
		...
		if (this.rotateControl) { p.orbitControl() }
		...
		if (this.particleA.velocity.mag().toFixed(2) == 0 && this.particleB.velocity.mag().toFixed(2) == 0) {
			this.paused = true;
		}
...
class Particle {
	...
	update(p, sf) {
	  ...
	  this.position.add(p5.Vector.mult(this.velocity, (1/p.getTargetFrameRate()) * sf));
